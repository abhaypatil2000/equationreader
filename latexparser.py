...
def util_function(content):
    ...
    while i<n :
        #i=i+1
        #print(i)
        if(flag==0):  #flag=0 implies no special functions like frac and sqrt
            #i=i+1
            ...
            if(content[i:i+2]==" \\n"):
                ...
                i=i+1 # check for i++
            '''
            if(content[i]!='\\' and content[i]!='^'): #write all chars except \ and ^
                ...
                temp=0
                li=['{']
                while(len(li)>0):  #getting contents inside {} to call the function recursively
